...
class Auth extends CI_Controller {
	...
	private function _login()
	{
		...
		$admin = $this->db->get_where('prci_user', ['email' => $email])->row_array();
		$admin1 = $this->db->get_where('prci_user', ['username' => $email])->row_array();
	...
	public function daftar()
	{
		...
		$this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[prci_user.email]',[
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->insert('prci_user', $data);
			$this->db->insert('prci_user_token', $user_token);
	...
	public function verify() {
		...
		if ($user) {
			$user_token = $this->db->get_where('prci_user_token', ['token' => $token])->row_array();
			if ($user_token) {
				if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
					$this->db->set('is_active', 1);
					$this->db->where('email', $email);
					$this->db->update('prci_user');
					$this->db->delete('prci_user_token', ['email' => $email]);
					...
					$this->db->delete('prci_user', ['email' => $email]);
					$this->db->delete('prci_user_token', ['email' => $email]);
	...
	public function daftaradmin()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->insert('prci_user_admin', $data);
	...
	public function gantipassword(){
		...
		$role = $this->db->get_where('prci_user_role', ['id' => $role_id])->row_array();
		$user = $this->db->get_where('prci_user', ['email' => $email])->row_array();
		$dbuser = 'prci_user';
	...
	public function forgotPassword(){
		...
		if ($this->form_validation->run() == false) {
			...
			$user = $this->db->get_where('prci_user', ['email' => $email, 'is_active' => 1])->row_array();
			if ($user) {
				$token = substr(md5(mt_rand()), 0, 32);
				$user_token = [
					'email' => $email,
